...
export default {
  ...
  methods: {
    ...
    startUpdate() {
      if (this.top < this.uplimit) {
        ...
        if (
          ...
          store.state.passDownlimit === config.land.top
        ) {
          game.score();
        }
        this.lastDownlimit = this.downlimit;
    ...
    listenGameEvent() {
      ...
      game.on("updownLimit", () => {
        this.downlimit =
          store.state.passDownlimit - this.height + 2 * positionfix;
    ...
    updateBirdImg() {
      if (this.gamestate === "over" || this.gamestate === "stop") {
